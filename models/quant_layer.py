...
class QuantConv2d(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=False, w_bit=8, a_bit=8):
        super(QuantConv2d, self).__init__(in_channels, out_channels, kernel_size, stride, padding, dilation, groups,
                                          bias)
        self.layer_type = 'QuantConv2d'
        self.wbit = w_bit
        self.abit = a_bit
        self.weight_quant = weight_quantize_fn(w_bit=self.wbit, power=True)
        self.act_grid = build_power_value(self.abit, additive=True)
        self.act_alq = act_quantization(self.abit, self.act_grid, power=True)
